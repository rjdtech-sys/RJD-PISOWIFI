...
async function protectInterfaceFromNetworkd(interface) {
  const dev = String(interface || '').trim();
  if (!dev || !/^[a-zA-Z0-9_.:-]+$/.test(dev)) return;

  try {
    const networkDir = '/run/systemd/network';
    if (!fs.existsSync(networkDir)) fs.mkdirSync(networkDir, { recursive: true });
    const safeName = dev.replace(/[^a-zA-Z0-9_.-]/g, '_');
    const configPath = path.join(networkDir, `00-rjd-${safeName}.network`);
    const config = `[Match]\nName=${dev}\n\n[Link]\nRequiredForOnline=no\n\n[Network]\nDHCP=no\nLinkLocalAddressing=no\nIPv6AcceptRA=no\nKeepConfiguration=static\n`;
    fs.writeFileSync(configPath, config);
    await execPromise('networkctl reload').catch(() => {});
  } catch (e) {
    console.warn(`[NET] Could not protect ${dev} from systemd-networkd: ${e.message}`);
  }
}
...
async function initFirewall() {
  ...
  try {
    ...
    const actualLan = bridge
      ? bridge.name
      : (
          ifaces.find(i => i.type === 'wifi' && i.status === 'up' && !isVirtualInterfaceName(i.name))?.name ||
          ifaces.find(i =>
            i.name !== primaryWan &&
            i.name !== 'lo' &&
            i.status === 'up' &&
            !isVirtualInterfaceName(i.name) &&
            (i.type === 'ethernet' || i.type === 'vlan')
          )?.name ||
          null
        );
    ...
    const wan = primaryWan;
    const allInterfaces = ifaces.filter(i => i.name !== wan && i.status === 'up' && !isVirtualInterfaceName(i.name));
    let redirectedInterfaceCount = 0;
    for (const lan of allInterfaces) {
      ...
      redirectedInterfaceCount++;
    }
    
    console.log(`[NET] Firewall ready. WAN: ${wan}. Portal redirects applied to ${redirectedInterfaceCount} interfaces.`);
  } catch (e) {
...
async function createVlan({ parent, id, name }) {
  ...
  try {
    ...
    await protectInterfaceFromNetworkd(finalName);
...
const bridgeRecoveryTimers = new Map();

function scheduleBridgeMemberRecovery(bridgeName, members) {
  const expectedMembers = [...new Set((members || []).map(member => String(member || '').trim()).filter(Boolean))];
  if (!bridgeName || expectedMembers.length === 0 || bridgeRecoveryTimers.has(bridgeName)) return;
  ...
  const timer = setInterval(async () => {
    ...
    for (const member of expectedMembers) {
      ...
      try {
        const { stdout } = await execPromise(`ip -j link show ${member}`);
        const info = JSON.parse(stdout || '[]')[0];
        if (info?.master === bridgeName) continue;

        console.log(`[NET] Late bridge member detected: ${member}. Attaching to ${bridgeName}...`);
        await protectInterfaceFromNetworkd(member);
        ...
        console.log(`[NET] Bridge member ${member} recovered on ${bridgeName}.`);
      } catch (e) {
        missingCount++;
      }
    }
    ...
    if (missingCount === 0 || attempts >= maxAttempts) {
      clearInterval(timer);
      bridgeRecoveryTimers.delete(bridgeName);
      if (attempts >= maxAttempts && missingCount > 0) {
        console.warn(`[NET] Timed out waiting for ${missingCount} bridge member(s) on ${bridgeName}.`);
      }
    }
  }, 5000);

  if (timer.unref) timer.unref();
  bridgeRecoveryTimers.set(bridgeName, timer);
}
...
async function createBridge({ name, members, stp }) {
  ...
  try {
    ...
    await protectInterfaceFromNetworkd(name);
    ...
    if (missingMembers.length > 0) {
      scheduleBridgeMemberRecovery(name, missingMembers);
    }
...
async function setupHotspot(config, skipRestart = false) {
  ...
  try {
    ...
    await protectInterfaceFromNetworkd(interface);
...
async function getLanInterface() {
  ...
  const lan = interfaces.find(i =>
    i.name !== (defaultWan || wanName) &&
    !isVirtualInterfaceName(i.name) &&
    (i.type === 'wifi' || i.type === 'ethernet' || i.type === 'vlan')
  );
...
async function ensureIFBDevice(ifbName = 'ifb0') {
  ...
  try {
    ...
    // systemd-networkd can try DHCPv6/RA handling on IFB devices and crash on
    // some Armbian builds. IFB is only used for traffic shaping, so IPv6 is not
    // useful here.
    await execPromise(`sysctl -w net.ipv6.conf.${ifbName}.disable_ipv6=1`).catch(() => {});
    await protectInterfaceFromNetworkd(ifbName);
...
async function initQoS(interface, discipline = 'cake') {
  if (!interface) {
    console.warn('[QoS] No LAN interface detected. Skipping QoS initialization.');
    return;
  }

  if (isVirtualInterfaceName(interface)) {
    console.warn(`[QoS] ${interface} is a virtual interface. Skipping QoS initialization.`);
    return;
  }

  try {
    await execPromise(`ip link show ${interface}`);
  } catch (e) {
    console.warn(`[QoS] Interface ${interface} not found. Skipping QoS initialization.`);
    return;
  }
...
async function configureWanDhcp(iface, isVlan = false) {
  ...
  try {
    const networkdActive = await execPromise('systemctl is-active systemd-networkd 2>/dev/null')
      .then(r => r.stdout.trim() === 'active')
      .catch(() => false);

    if (networkdActive && !isVlan) {
      console.log(`[WAN] Requesting non-destructive systemd-networkd DHCP renewal on ${dev}...`);
      await execPromise(`ip link set dev ${dev} up`).catch(() => {});
      await execPromise(`networkctl renew ${dev}`).catch(() => {});

      for (let attempt = 0; attempt < 12; attempt++) {
        await new Promise(r => setTimeout(r, 1000));
        const status = await getWanStatus(dev);
        const gateway = await getWanGateway(dev);
        if (status.status === 'up' && status.ip && gateway) {
          console.log(`[WAN] systemd-networkd renewed ${dev}: IP=${status.ip}, GW=${gateway}`);
          return { success: true, ip: status.ip, gateway, status: status.status };
        }
      }

      console.warn(`[WAN] systemd-networkd renewal did not restore ${dev}; falling back to direct DHCP client.`);
    }
...
async function removeWanConfig(iface) {
  ...
  if (!dev) return { success: false, error: 'Interface name required' };
  try {
    const protectedRows = await db.all('SELECT interface FROM hotspots WHERE enabled = 1').catch(() => []);
    const isHotspotInterface = protectedRows.some(row => String(row.interface || '') === dev);
    const bridgeRows = await db.all('SELECT name, members FROM bridges').catch(() => []);
    const isBridgeInterface = bridgeRows.some(row => {
      if (String(row.name || '') === dev) return true;
      try {
        const members = JSON.parse(row.members || '[]');
        return Array.isArray(members) && members.includes(dev);
      } catch (e) {
        return false;
      }
    });

    if (isHotspotInterface || isBridgeInterface) {
      console.warn(`[WAN] Refusing to remove config from LAN/hotspot interface ${dev}`);
      await removeWanFirewallRules(dev);
      return { success: true, skipped: true, reason: 'protected_lan_interface' };
    }
...
async function applyWanConfig(wan) {
  ...
  if (type === 'dhcp') {
    const currentStatus = await getWanStatus(name);
    const currentGateway = await getWanGateway(name);
    if (currentStatus.status === 'up' && currentStatus.ip && currentGateway) {
      console.log(`[WAN] ${name} already has DHCP IP ${currentStatus.ip} and gateway ${currentGateway}. Preserving active lease.`);
