...
async function getInterfaces() {
  try {
    ...
    return data.map(iface => {
      ...
      let type = 'ethernet';
      if (ifname.startsWith('wl') || ifname.startsWith('ap') || ifname.startsWith('ra')) {
        type = 'wifi';
...
async function autoProvisionNetwork() {
  ...
  try {
    ...
    const wlanInterface = lanMembers.find(m => m.startsWith('wl') || m.startsWith('ra') || m.startsWith('ap'));
