...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    // Get expired pool settings to include in PPPoE server pool range
    const { pool: expiredPool } = await getPPPoEExpiredSettings();
    let poolStart = ip_pool_start || '10';
    let poolEnd = ip_pool_end || '254';
    
    // If expired pool is configured, ensure PPPoE server pool includes both ranges
    if (expiredPool && expiredPool.ip_pool_start && expiredPool.ip_pool_end) {
      const serverStartInt = ipToInt(poolStart);
      const serverEndInt = ipToInt(poolEnd);
      const expiredStartInt = ipToInt(expiredPool.ip_pool_start);
      const expiredEndInt = ipToInt(expiredPool.ip_pool_end);
      
      if (serverStartInt !== null && serverEndInt !== null && expiredStartInt !== null && expiredEndInt !== null) {
        // Check if expired pool is outside server pool
        const serverMin = Math.min(serverStartInt, serverEndInt);
        const serverMax = Math.max(serverStartInt, serverEndInt);
        const expiredMin = Math.min(expiredStartInt, expiredEndInt);
        const expiredMax = Math.max(expiredStartInt, expiredEndInt);
        
        // If expired pool is in a different subnet/range, we need to use a combined range
        // or at least ensure the expired pool is reachable
        if (expiredMin < serverMin || expiredMax > serverMax) {
          console.log(`[PPPoE-Server] Expired pool ${expiredPool.ip_pool_start}-${expiredPool.ip_pool_end} is outside main pool ${poolStart}-${poolEnd}`);
          console.log(`[PPPoE-Server] Using combined pool range to support expired users`);
          poolStart = intToIp(Math.min(serverMin, expiredMin));
          poolEnd = intToIp(Math.max(serverMax, expiredMax));
        }
      }
    }
    
    const cmd = `nohup pppoe-server -I ${targetIface} -L ${local_ip} -R ${poolStart}-${poolEnd} -N 253 ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
