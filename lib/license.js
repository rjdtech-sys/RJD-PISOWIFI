...
class LicenseManager {
  ...
  isTrialLicense(license) {
    return Boolean(
      license && (
        ...
        String(license.license_key || license.licenseKey || '').startsWith('RJD-TRIAL-')
      )
    );
  }

  async cacheCloudLicense(hardwareId, license) {
    const isTrial = this.isTrialLicense(license);
    ...
    await db.run(
      `INSERT INTO license_info (
        hardware_id, license_key, is_active, is_revoked, max_online_users,
        activated_at, expires_at, trial_started_at, trial_expires_at, created_at
      ) VALUES (?, ?, 1, 0, ?, ?, ?, ?, ?, ?)
      ...
      [
        hardwareId,
        license.license_key,
        this.normalizeMaxOnlineUsers(license.max_online_users),
        activatedAt,
        expiresAt,
  ...
  verificationResult(license) {
    ...
    return {
      ...
      isTrial,
      licenseType: isTrial ? 'trial' : (license.license_type || 'basic'),
    ...
    try {
      ...
      if (existingHardware) {
        if (existingHardware.license_key === licenseKey) {
          ...
          await this.cacheCloudLicense(hardwareId, existingHardware);
      ...
      await this.cacheCloudLicense(hardwareId, updatedLicense);
  ...
  async fetchAndCacheLicense(hardwareId) {
    ...
    try {
      ...
      await this.cacheCloudLicense(hardwareId, license);
  ...
  async verifyLicense() {
    if (!this.supabase) {
      ...
      try {
        ...
        if (localLicense && !this.isExpired(localLicense)) {
          ...
          return this.verificationResult(localLicense);
        } else {
    ...
    try {
      ...
      // Fetch the current active license. Historical trials may remain on the hardware.
      const { data: license, error } = await this.supabase
        .from('licenses')
        .select('*')
        .eq('hardware_id', hardwareId)
      ...
      await this.cacheCloudLicense(hardwareId, license);
      return this.verificationResult(license);
