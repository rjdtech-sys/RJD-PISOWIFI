...
class EdgeSync {
  ...
  async getLocalLicenseKey() {
    if (!this.hardwareId) return null;
    try {
      const row = await db.get(
        `SELECT license_key
         FROM license_info
         WHERE hardware_id = ? AND is_active = 1 AND is_revoked = 0
         LIMIT 1`,
        [this.hardwareId]
      );
  ...
  async syncMachineStatus(status) {
    ...
    try {
      ...
      const licenseInfo = await db.get('SELECT trial_started_at, trial_expires_at FROM license_info WHERE hardware_id = ? LIMIT 1', [this.hardwareId]).catch(() => null);

      const licenseKey = await this.getLocalLicenseKey();
      if (!licenseKey) throw new Error('No active local machine license is available for status sync');

      const { data, error } = await this.supabase.rpc('sync_edge_machine_status', {
        p_hardware_id: this.hardwareId,
        p_license_key: licenseKey,
        p_status: status,
        p_cpu_temp: metrics.cpuTemp,
        p_uptime_seconds: metrics.uptime,
        p_active_sessions_count: metrics.activeSessions,
        p_trial_started_at: licenseInfo?.trial_started_at || null,
        p_trial_expires_at: licenseInfo?.trial_expires_at || null
      });
      if (error) throw error;

      if (data?.machine_id) this.machineId = data.machine_id;
      if (data?.vendor_id && this.vendorId !== data.vendor_id) {
        this.vendorId = data.vendor_id;
        this.saveLocalVendorId(data.vendor_id);
      }
   ...
   */
  async recordSale(saleData, options = {}) {
    ...
    const normalizedSale = {
      ...saleData,
      event_id: saleData.event_id || `edge-${this.hardwareId || 'unknown'}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
    };
    if (!this.supabase || !this.machineId) {
      // Queue sale if offline or not linked
      if (queueOnFailure) this.queueSync('sale', normalizedSale);
      return false;
    }

    try {
      const licenseKey = await this.getLocalLicenseKey();
      if (!licenseKey) throw new Error('No active local machine license is available for sale sync');

      const notes = typeof normalizedSale.metadata === 'object'
        ? JSON.stringify(normalizedSale.metadata)
        : normalizedSale.metadata || null;

      const { error } = await this.supabase.rpc('record_edge_sale', {
        p_hardware_id: this.hardwareId,
        p_license_key: licenseKey,
        p_source_event_id: normalizedSale.event_id,
        p_amount: normalizedSale.amount,
        p_transaction_type: normalizedSale.transaction_type || 'coin_insert',
        p_session_duration: normalizedSale.session_duration || null,
        p_customer_mac: normalizedSale.customer_mac || null,
        p_customer_ip: normalizedSale.customer_ip || null,
        p_notes: notes
      });
  ...
  async processQueue() {
    ...
    for (const item of itemsToProcess) {
      ...
      try {
        if (item.type === 'sale') {
            success = await this.recordSale(item.data, { queueOnFailure: false });
        }
      } catch (e) { /* ignore */ }
  ...
  async loadLocalIdentity() {
    try {
        const [vendorRow, machineRow] = await Promise.all([
          db.get('SELECT value FROM config WHERE key = ?', ['cloud_vendor_id']),
          db.get('SELECT value FROM config WHERE key = ?', ['cloud_machine_id'])
        ]);
        if (vendorRow && vendorRow.value) {
            this.vendorId = vendorRow.value;
            console.log(`[EdgeSync] Loaded local vendor ID: ${this.vendorId}`);
        }
        if (machineRow && machineRow.value) {
            this.machineId = machineRow.value;
            console.log(`[EdgeSync] Loaded local machine ID: ${this.machineId}`);
        }
    } catch (e) { /* ignore */ }
  }
  ...
  async syncClientsToCloud() {
      ...
      try {
          ...
          const licenseKey = await this.getLocalLicenseKey();
          if (!licenseKey) {
              console.warn('[EdgeSync] Skipping client sync: no active local machine license.');
              return;
          }

          // Upsert both the rich wifi_devices record and the compatibility clients record.
          try {
              for (const update of filteredUpdates) {
                  const { error } = await this.supabase.rpc('sync_edge_client', {
                      p_hardware_id: this.hardwareId,
                      p_license_key: licenseKey,
                      p_session_token: update.session_token,
                      p_mac_address: update.mac_address,
                      p_ip_address: update.ip_address || null,
                      p_device_name: update.device_name || null,
                      p_remaining_seconds: update.remaining_seconds || 0,
                      p_total_paid: update.total_paid || 0,
                      p_is_connected: update.is_connected === true
                  });
                  if (error) throw error;
               ...
               console.error('[EdgeSync] Client sync RPC failed:', upsertErr.message);
          }
   ...
   async checkPendingCommands() {
       ...
       try {
           ...
           if (error) {
               ...
               if (message.includes('machine_commands') || message.includes('schema cache')) {
                   if (!this._machineCommandsMissingLogged) {
                       console.warn('[EdgeSync] Remote commands disabled: machine_commands table is missing in Supabase.');
                       this._machineCommandsMissingLogged = true;
                   }
                   return;
               }
